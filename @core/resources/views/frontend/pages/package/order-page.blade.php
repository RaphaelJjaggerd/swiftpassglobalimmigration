                        ...
                        <div class="order-tab-wrap">
                            <nav>
                                <div class="nav nav-tabs" role="tablist">
                                    ...
                                    @endif
                                    <a class="nav-item nav-link  @if (auth()->check()) active @else disabled @endif"
                                ...
                                @if (!auth()->check())
                                    <div class="tab-pane fade show active" id="nav-home" role="tabpanel">
                                        @if (get_static_option('disable_guest_mode_for_package_module'))
                                            <div
                                                class="checkout-type margin-bottom-30  @if (auth()->check()) d-none @endif">
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input" id="guest_logout"
                                                        name="checkout_type">
                                                    <label class="custom-control-label"
                                                        for="guest_logout">{{ __('Guest Order') }}</label>
                                                </div>
                                            </div>
                                        @endif
                                        @if (!auth()->check())
                                                ...
                                                <form action="{{ route('user.login') }}" method="post"
                                                    ...
                                                    <div class="form-group btn-wrapper">
                                                        <button type="submit" id="login_btn"
                                                            class="submit-btn">{{ __('Login') }}</button>
                                                    </div>
                                                    <div class="row mb-4 rmber-area">
                                                        ...
                                                        <div class="col-6 text-right">
                                                            <a class="d-block"
                                                                href="{{ route('user.register') }}">{{ __('Create new account?') }}</a>
                                                            <a
                                                                href="{{ route('user.forget.password') }}">{{ __('Forgot Password?') }}</a>
                                                        </div>
                                                    </div>
                                                </form>
                                            </div>
                                        ...
                                        @if (!auth()->check())
                                            <div class="next-step">
                                                <button class="next-step-btn btn-boxed d-none"
                                                    type="button">{{ __('Next Step') }}</button>
                                            </div>
                                        @endif
                                    </div>
                                @endif
                                <div class="tab-pane fade @if (auth()->check()) show active @endif"
                                    id="nav-profile" role="tabpanel">
                                    <form action="{{ route('frontend.order.message') }}" method="post"
                        ...
                        <div class="single-price-plan-01 style-02 active">
                            <div class="price-header">
                                ...
                                <div class="price-wrap">
                                    <span class="price">{{ amount_with_currency_symbol($order_details->price) }}</span>
                                    {{-- <span class="month">{{ $order_details->type }}</span> --}}
                                </div>
                            </div>
                            ...
                            <div class="btn-wrapper text-center">
                                @if (!empty($order_details->url_status))
                                    <a class="order-btn boxed-btn"
                                        href="{{ route('frontend.plan.order', $order_details->id) }}"><span
                                            class="order-btn-text">{{ $order_details->btn_text }}</span> </a>
                                @else
                                    <a class="order-btn boxed-btn"
                                        href="{{ $order_details->btn_url }}">{{ $order_details->btn_text }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
