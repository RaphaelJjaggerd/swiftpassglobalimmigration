                    ...
                    <div class="enroll-area-wrapper">
                        @if (auth()->guard('web')->check())
                            ...
                            <div class="enroll-form-wrapper">
                                <x-error-msg />
                                <x-flash-msg />
                                <h4 class="amount-title">{{ __('Payable Amount') }} <span
                                        class="price">{{ amount_with_currency_symbol($course->price) }}</span></h4>
                                <form action="{{ route('frontend.course.enroll.submit') }}" method="post"
                                    class="appointment-booking-form" id="appointment_rating_form"
                                    ...
                                    @if (!empty($course->price) && $course->price != 0)
                                        ...
                                        @if (!empty(get_static_option('manual_payment_gateway')))
                                            <div
                                                class="form-group manual_payment_transaction_field @if (get_static_option('site_default_payment_gateway') === 'manual_payment') d-block @endif ">
                                                <div class="label mb-2">{{ __('Attach your bank Document') }}</div>
                                                <input class="form-control btn btn-primary btn-sm pb-2 pt-2" type="file"
                                    ...
                                    <div class="btn-wrapper">
                                        <button type="submit"
                                            class="btn-boxed enroll">{{ get_static_option('course_single_' . $user_select_lang_slug . '_enroll_button_text') }}</button>
                                    </div>
                                </form>
                            </div>
                        @else
                            @include('frontend.partials.ajax-login-form', [
                                'title' => __('Login to enroll'),
                            ])
                        @endif
                    </div>
                </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.status == 'success') {
                            ...
                            $('.enroll-form-wrapper .amount-title .price').html(data.amount +
                                '<del>' + oldPrice + '</del>');
                        }
