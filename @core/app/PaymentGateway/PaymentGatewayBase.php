...
abstract class PaymentGatewayBase {
  ...
  protected static function get_amount_in_kes($amount) {
    if (self::global_currency() === 'KES') {
      return $amount;
    }
    $payable_amount = self::make_amount_in_kes($amount, self::global_currency());
    if ($payable_amount < 1) {
      $called_class_name = get_called_class();
      $instance = new $called_class_name();
  ...
  protected static function make_amount_in_kes($amount, $currency) {
    $output = 0;
    $all_currency = GlobalCurrency::script_currency_list();
    foreach ($all_currency as $cur => $symbol) {
      if ($cur === 'KES') {
