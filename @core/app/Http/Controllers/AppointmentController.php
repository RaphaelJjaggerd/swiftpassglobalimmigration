...
use Carbon\Carbon;
use App\AppointmentBookingSlot;
...
class AppointmentController extends Controller {
  ...
  public function appointment_store(Request $request) {
    $this->validate($request, [
      ...
      'appointment_dates' => 'required|string',
    ]);
    ...
    try {
      ...
      // Check and delete past booking dates
  ...
  public function appointment_update(Request $request) {
    ...
    try {
      ...
      foreach ($dates as $date) {
        ...
        $appointment->bookingDates()->updateOrCreate(['date' => $date]);
      }
  ...
  public function deletePastBookingDates(Appointment $appointment) {
    // Get the current date and time
    $now = Carbon::now();

    // Delete past booking dates associated with the appointment
    $appointment->bookingDates()->where('date', '<', $now)->delete();
  }
}
