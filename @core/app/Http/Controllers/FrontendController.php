...
ITEM;
  ...
  public function subscribe_newsletter(Request $request) {
    ...
    return view('frontend.pages.newsletter-success')->with(['msg' => __('Thanks for Subscribe Our Newsletter'), 'type' => 'success']);
  }
  ...
  public function order_confirm($id) {
    ...
    return view('frontend.payment.order-confirm')->with(['order_details' => $order_details, 'package_details' => $package_details]);
  }
  ...
  public function apply_order_coupon(Request $request) {
    $order = Order::findOrFail($request->order_id);
    $return_val = ['msg' => __('Enter a valid coupon'), 'status' => 'danger'];
    ...
    if ($coupon_details) {
      ...
      if ($coupon_details->discount_type === 'percentage') {
        $discount_bal = ($order->package_price / 100) * (int) $coupon_details->discount;
        $discounted_amount = $order->package_price - $discount_bal;
      } elseif ($coupon_details->discount_type === 'amount') {
        $discounted_amount = $order->package_price - (int) $coupon_details->discount;
      }

      // Update the order with the discounted amount
      // $order->update(['discounted_amount' => $discounted_amount]);

      $order->discounted_amount = $discounted_amount;
      $order->save();

      $newPrice = $order->package_price - $discounted_amount;

      $return_val['msg'] = __('Coupon applied successfully');
      $return_val['amount'] = amount_with_currency_symbol($newPrice);
  ...
  public function apply_appointment_coupon(Request $request) {
    $order = Order::findOrFail($request->order_id);
    $return_val = ['msg' => __('Enter a valid coupon'), 'status' => 'danger'];
